...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    1.5) Query design intelligence: if `skills/design-intelligence/` exists, search for relevant styles, color palettes, and typography for the project's industry/context. Use results to inform (not dictate) your aesthetic direction.
        - `python3 skills/design-intelligence/scripts/search.py "<industry/context>" --domain product -n 3` for style recommendations
        - `python3 skills/design-intelligence/scripts/search.py "<context>" --domain color -n 3` for color palettes
        - `python3 skills/design-intelligence/scripts/search.py "<mood>" --domain typography -n 3` for font pairings
        - Or read curated references directly: `skills/design-intelligence/references/styles-catalog.md`, `color-palettes.md`, `typography-pairings.md`
  ...
  <Tool_Usage>
    ...
    - Use design-intelligence data for informed style/color/typography decisions when available:
      Read `skills/design-intelligence/references/` for curated guides, or
      run `python3 skills/design-intelligence/scripts/search.py` for targeted BM25 queries.
    - Use Write/Edit for creating and modifying components.
    - Use Bash to run dev server or build to verify implementation.
    <External_Consultation>
      When a second opinion would improve quality, spawn a sibling agent:
      - Use the designer agent for UI/UX cross-validation
      - Use team mode for large-scale frontend work
