...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - For finding all usages of a symbol, use LSP-style cross-reference tools when available.
  ...
  <Investigation_Protocol>
    ...
    3) Cross-validate findings across multiple tools (Grep results vs Glob results vs structural search).
  ...
  <Context_Budget>
    ...
    - Before reading a file with Read, check its size with a quick `wc -l` via Bash.
    - For files >200 lines, get the file outline first (function/class signatures), then only read specific sections with `offset`/`limit` parameters on Read.
    - For files >500 lines, ALWAYS use an outline approach instead of full Read unless the caller specifically asked for full file content.
    ...
    - Prefer structural tools (outline-style scans, Grep) over full Read whenever possible -- they return only the relevant information without consuming context on boilerplate.
  </Context_Budget>
  ...
  <Tool_Usage>
    ...
    - Prefer the right tool for the job: Grep for text patterns, Glob for file patterns, git for history.
  </Tool_Usage>
  ...
  <Failure_Modes_To_Avoid>
    ...
    - Reading entire large files: Reading a 3000-line file when an outline would suffice. Always check size first and use targeted Read with offset/limit.
  </Failure_Modes_To_Avoid>
