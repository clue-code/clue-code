...
- Cancel with `/clue-code:cancel` at any time; progress is preserved for resume
</Execution_Policy>

<Steps>
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - **If ralplan consensus plan exists** (`.clue-code/plans/ralplan-*.md` or `.clue-code/plans/consensus-*.md` from the 3-stage pipeline): Skip BOTH Phase 0 and Phase 1 — jump directly to Phase 2 (Execution). The plan has already been Planner/Architect/Critic validated.
   - **If deep-interview spec exists** (`.clue-code/specs/deep-interview-*.md`): Skip analyst+architect expansion, use the pre-validated spec directly as Phase 0 output. Continue to Phase 1 (Planning).
   ...
   - **Otherwise**: Analyst (L2) extracts requirements, Architect (L2) creates technical specification
   - Output: `.clue-code/autopilot/spec.md`
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Architect (L2): Create plan (direct mode, no interview)
   - Critic (L3): Validate plan
   - Output: `.clue-code/plans/autopilot-impl.md`

3. **Phase 2 - Execution**: Implement the plan using Ralph + Ultrawork
   - Executor (L0): Simple tasks
   - Executor (L1): Standard tasks
   - Executor (L2): Complex tasks
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   - Remove `.clue-code/state/autopilot-state.json`, `ralph-state.json`, `ultrawork-state.json`, `ultraqa-state.json`
   - Run `/clue-code:cancel` for clean exit
</Steps>

<Tool_Usage>
- Delegate to the architect agent for Phase 4 architecture validation
- Delegate to the security-reviewer agent for Phase 4 security review
- Delegate to the code-reviewer agent for Phase 4 quality review
- Agents form their own analysis first, then spawn cross-validation agents
...
Optional settings in `clue-code.yaml`:
...
If autopilot was cancelled or failed, run `/clue-code:autopilot` again to resume from where it stopped.
...
**Stuck in a phase?** Check task list for blocked tasks, review `.clue-code/state/autopilot-state.json`, or cancel and resume.
...
If a deep-interview spec already exists at `.clue-code/specs/deep-interview-*.md`, autopilot uses it directly as Phase 0 output (the spec has already been mathematically validated for clarity).
...
When autopilot detects a ralplan consensus plan (`.clue-code/plans/ralplan-*.md` or `.clue-code/plans/consensus-*.md`), it skips both Phase 0 (Expansion) and Phase 1 (Planning) because the plan has already been:
