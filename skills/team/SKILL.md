---
name: team
description: N coordinated agents on shared task list using CLUE CODE native teams
aliases: []
level: 4
---
...
Spawn N coordinated agents working on a shared task list using CLUE CODE's native team primitives. Provides built-in team management, inter-agent messaging, and task dependencies — no external dependencies required.

## Usage

```
/clue-code:team N:agent-type "task description"
/clue-code:team "task description"
/clue-code:team ralph "task description"
```

### Parameters

- **N** - Number of teammate agents (1-20). Optional; defaults to auto-sizing based on task decomposition.
- **agent-type** - CLUE CODE agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
...
```bash
/clue-code:team 5:executor "fix all TypeScript errors across the project"
/clue-code:team 3:debugger "fix build errors in src/"
/clue-code:team 4:designer "implement responsive layouts for all page components"
/clue-code:team "refactor the auth module with security review"
/clue-code:team ralph "build a complete REST API for user management"
# With Codex CLI workers (requires: npm install -g @openai/codex)
/clue-code:team 2:codex "review architecture and suggest improvements"
# With Gemini CLI workers (requires: npm install -g @google/gemini-cli)
/clue-code:team 2:gemini "redesign the UI components"
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              ...
              +-- Completion
                      ...
                      -> rm .clue-code/state/team-state.json
```

**Storage layout (managed by CLUE CODE orchestrator):**
```
~/.clue-code/
  teams/fix-ts-errors/
...
|-------|----------------|-----------------|-------------------|
| **team-plan** | `explore` (L0), `planner` (L2) | `analyst` (L2), `architect` (L2) | Use `analyst` for unclear requirements. Use `architect` for systems with complex boundaries. |
| **team-prd** | `analyst` (L2) | `critic` (L3) | Use `critic` to challenge scope. |
| **team-exec** | `executor` (L1) | `executor` (L2), `debugger` (L1), `designer` (L1), `writer` (L0), `test-engineer` (L1) | Match agent to subtask type. Use `executor` (L2) for complex autonomous work, `designer` for UI, `debugger` for compilation issues, `writer` for docs, `test-engineer` for test creation. |
| **team-verify** | `verifier` (L1) | `test-engineer` (L1), `security-reviewer` (L3), `code-reviewer` (L2) | Always run `verifier`. Add `security-reviewer` for auth/crypto changes. Add `code-reviewer` for >20 files or architectural changes. `code-reviewer` also covers style/formatting checks. |
| **team-fix** | `executor` (L1) | `debugger` (L1), `executor` (L2) | Use `debugger` for type/build errors and regression isolation. Use `executor` (L2) for complex multi-file fixes. |
...
3. **Cost mode affects model tier.** In downgrade: L2 agents to L1, L1 to L0 where quality permits. `team-verify` always uses at least L1.
4. **Risk level escalates review.** Security-sensitive or >20 file changes must include `security-reviewer` + `code-reviewer` (L2/L3) in `team-verify`.
...
The lead writes handoffs to `.clue-code/handoffs/<stage-name>.md`.
...
3. **On team cancellation, handoffs survive** in `.clue-code/handoffs/` for session resume. They are not deleted by team deletion.
...
- **Resume:** restart from the last non-terminal stage using staged state + live task status. Read `.clue-code/handoffs/` to recover stage transition context.
- **Cancel:** `/clue-code:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.clue-code/handoffs/` are preserved for potential resume.
...
- Extract **agent-type**, validate it maps to a known CLUE CODE subagent
...
Call the orchestrator's TeamCreate primitive with a slug derived from the task:
...
{
  "team_name": "fix-ts-errors",
  "team_file_path": "~/.clue-code/teams/fix-ts-errors/config.json",
...
Write CLUE CODE state using `state_write` for proper session-scoped persistence:
...
> **Note:** `state_write` transports all values as strings. Consumers must coerce `agent_count`, `fix_loop_count`, `max_fix_loops` to numbers and `linked_ralph` to boolean when reading state.
...
Call TaskCreate for each subtask. Set dependencies via TaskUpdate using `addBlockedBy`.
...
Spawn N teammates with `team_name` and `name` parameters. Each teammate gets the team worker preamble (see below) plus their specific assignment.
...
On every stage transition, update CLUE CODE state:
...
4. **Delete team** -- Call TeamDelete to clean up:
   ...
   ```
5. **Clean CLUE CODE state** -- Remove `.clue-code/state/team-state.json`
...
- NEVER run team spawning/orchestration skills or commands (for example `$team`, `$ultrawork`, `$autopilot`, `$ralph`, `clue-code team ...`)
...
- `codex_worker`: Emphasize CLI API lifecycle (`clue-code team api ... --json`) and explicit failure ACKs with stderr.
...
```bash
node "${CLUE_CODE_HOME}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```
...
The team skill supports **hybrid execution** combining native agent teammates with external CLI workers (Codex CLI and Gemini CLI). Both types can make code changes -- they differ in capabilities and cost. These are standalone CLI tools.
...
|---------------|----------|-------------|
| `claude_worker` | Native CLUE CODE agent | Full tool access (Read/Write/Edit/Bash). Best for tasks needing the orchestrator's reasoning + iterative tool use. |
...
**Key difference from native teammates:**
- CLI workers operate via tmux, not the orchestrator's tool system
...
|-----------|-----------|-----|
| Iterative multi-step work | Native teammate | Needs tool-mediated iteration + team communication |
| Code review / security audit | CLI worker or specialist agent | Autonomous execution, good at structured analysis |
| Architecture analysis / planning | architect agent | Strong analytical reasoning with codebase access |
| Refactoring (well-scoped) | CLI worker or executor agent | Autonomous execution, good at structured transforms |
| UI/frontend implementation | designer agent | Design expertise, framework idioms |
| Large-scale documentation | writer agent | Writing expertise + large context for consistency |
| Build/test iteration loops | Native teammate | Needs Bash tool + iterative fix cycles |
| Tasks needing team coordination | Native teammate | Needs SendMessage for status updates |

### Example: Hybrid Team with CLI Workers

```
/clue-code:team 3:executor "refactor auth module with security review"

Task decomposition:
#1 [codex_worker] Security review of current auth code -> output to .clue-code/research/auth-security.md
...
The lead runs #1 (Codex security analysis), then #2 and #3 in parallel (Codex refactors backend, designer agent redesigns frontend), then #4 (native teammate handles test iteration), then #5 (Gemini final review).

### Pre-flight Analysis (Optional)

For large ambiguous tasks, run analysis before team creation:

1. Spawn the planner agent with task description + codebase context
...
- **Worker registration** -- Which workers are registered (from shadow registry / config.json)
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for tmux/CLI workers that cannot use the native team messaging tools.
...
4. If needed, spawn a replacement teammate with the same `team_name`/`name` flow

## Team + Ralph Composition

When the user invokes `/clue-code:team ralph`, says "team ralph", or combines both keywords, team mode wraps itself in Ralph's persistence loop. This provides:
...
1. User invokes `/clue-code:team ralph "task"`
...
4. If architect approves: both modes complete, run `/clue-code:cancel`
...
1. Check `~/.clue-code/teams/` for teams matching the task slug
...
The `/clue-code:cancel` skill handles team cleanup:
...
If teammates are unresponsive, TeamDelete may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.clue-code/teams/{team_name}/` and `~/.clue-code/tasks/{team_name}/`.

## Runtime V2 (Event-Driven)

When `CLUE_CODE_RUNTIME_V2=1` is set, the team runtime uses an event-driven architecture instead of the legacy done.json polling watchdog:
...
- **Event log**: All team events are appended to `.clue-code/state/team/{teamName}/events.jsonl`
- **Worker status files**: Workers write status to `.clue-code/state/team/{teamName}/workers/{name}/status.json`
...
When `CLUE_CODE_TEAM_SCALING_ENABLED=1` is set, the team supports mid-session scaling:
...
Optional settings via `clue-code.yaml`:
...
> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate orchestrator session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.

## State Cleanup

On successful completion:

1. TeamDelete handles all native team state:
   - Removes `~/.clue-code/teams/{team_name}/` (config)
   - Removes `~/.clue-code/tasks/{team_name}/` (all task files + lock)
2. CLUE CODE state cleanup:
   ```
   ...
   ```
3. Or run `/clue-code:cancel` which handles all cleanup automatically.
...
Workers can operate in isolated git worktrees to prevent file conflicts between concurrent workers.

### How It Works

1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.clue-code/worktrees/{team}/{worker}` with branch `clue-team/{teamName}/{workerName}`.
