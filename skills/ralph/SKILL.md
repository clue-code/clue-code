...
- Read `docs/agent-tiers.md` before first delegation to select correct agent tiers
- Deliver the full implementation: no scope reduction, no partial completion, no deleting tests to make them pass
</Execution_Policy>

<Steps>
1. **PRD Setup** (first iteration only):
   a. Check if `prd.json` exists (in project root or `.clue-code/`). If it already exists, read it and proceed to Step 2.
   b. If no `prd.json` exists, the system has auto-generated a scaffold. Read `.clue-code/prd.json`.
...
3. **Implement the current story**:
   - Delegate to specialist agents at appropriate tiers:
     - Simple lookups: L0 tier -- "What does this function return?"
     - Standard work: L1 tier -- "Add error handling to this module"
     - Complex analysis: L2 tier -- "Debug this race condition"
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   - <5 files, <100 lines with full tests: STANDARD tier minimum (architect-medium / L1)
   - Standard changes: STANDARD tier (architect-medium / L1)
   - >20 files or security/architectural changes: THOROUGH tier (architect / L2)
   - If `--critic=critic`, use the `critic` agent for the approval pass
   - If `--critic=codex`, run `clue-code ask codex --agent-prompt critic "..."` for the approval pass
...
7.5 **Mandatory Deslop Pass**:
   - Unless `{{PROMPT}}` contains `--no-deslop`, run the `clue-code:ai-slop-cleaner` skill in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/clue-code:cancel` to cleanly exit and clean up all state files
...
<Tool_Usage>
- Delegate to the architect agent for verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Delegate to the critic agent when `--critic=critic`
- Use `clue-code ask codex --agent-prompt critic "..."` when `--critic=codex`
...
```
delegate to executor (L0) "Add type export for UserConfig"
delegate to executor (L1) "Implement the caching layer for API responses"
delegate to executor (L2) "Refactor auth module to support OAuth2 flow"
```
...
```
delegate executor "Add type export" → wait →
delegate executor "Implement caching" → wait →
delegate executor "Refactor auth"
```
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/clue-code:cancel`
...
- [ ] `/clue-code:cancel` run for clean state cleanup
</Final_Checklist>
